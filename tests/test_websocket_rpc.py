...
def test_send_chat_falls_back_to_chat_history_after_timeout(monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        if method == "chat.history":
            return {
                "messages": [
                    {
                        "role": "user",
                        "content": [{"type": "text", "text": "Internal GUI context update."}],
                    },
                    {
                        "role": "assistant",
    ...
    def fake_wait_for_chat_completion(
        session_key: str,
        run_id: str,
        *,
...
def test_wait_for_chat_completion_uses_stream_text_when_final_payload_has_no_message(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    client = OpenClawRpcClient("http://gateway.test", gateway_token="token-123")
    events = iter(
    ...
    assert result["sessionKey"] == "agent:main:session-1"
    assert result["message"]["role"] == "assistant"
    assert result["message"]["content"][0]["text"] == "real reply"
...
def test_send_chat_history_fallback_ignores_previous_context_sync_reply(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    client = OpenClawRpcClient("http://gateway.test", gateway_token="token-123")
        ...
        if method == "chat.history":
            return {
                "messages": [
                    ...
                    {
                        "role": "assistant",
                        "content": [{"type": "text", "text": "Use /tmp/project as the workspace."}],
                    },
                    {
                        "role": "user",
                        "content": [{"type": "text", "text": "Look at the code and explain it."}],
                    },
                ]
            }
        raise AssertionError(f"Unexpected RPC method: {method}")
    ...
    with pytest.raises(GatewayTimeoutError):
        client.send_chat("session-1", "Look at the code and explain it.")


def test_history_match_does_not_skip_past_a_newer_user_message() -> None:
    history = {
        "messages": [
    ...
    assert (
        OpenClawRpcClient._latest_assistant_message_from_history(
            history,
            after_user_text="repeat",
        )
        is None
    )


def test_wait_for_chat_completion_raises_when_final_has_no_assistant_reply(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    client = OpenClawRpcClient("http://gateway.test", gateway_token="token-123")
    events = iter(
    ...
    monkeypatch.setattr(client, "_next_event", lambda predicate: next(events))
    monkeypatch.setattr(client, "_latest_assistant_message", lambda *args, **kwargs: None)

    with pytest.raises(GatewayMalformedResponseError):
        client.wait_for_chat_completion("session-1", "run-1")
